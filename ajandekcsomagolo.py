...
def festekszamlalo():
 ...
 hossz = float(input("Kélek add meg a doboz hosszát (pl.10.1) cm-ben: "))
szelesseg = float(input("Kélek add meg a doboz szélességét (pl.10.1) cm-ben: "))
magassag = float(input("Kélek add meg a doboz magasságát (pl.10.1) cm-ben: "))
szalag_hossz = float(input("Kélek add meg a rendelkezésre szalag hosszát (pl.10.1) m-ben: "))
...
print(f"Az ajandekok csomagolásához szükséges szalag {szukseges_szalag_meterben} méter")

if szalag_hossz < szukseges_szalag_meterben:
    print(f"A rendelkezésre álló szalag nem elegendő. Szükség van még {szukseges_szalag_meterben-szalag_hossz}")
else:
